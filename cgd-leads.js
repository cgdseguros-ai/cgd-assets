...
(function(){
  ...
  function runHorseAcross(){
    ...
    try{
      ...
      d.innerHTML = `
        <div class="cgdRaceScene" aria-hidden="true">
          <div class="cgdDust cgdDust1"></div>
          <div class="cgdDust cgdDust2"></div>
          <div class="cgdHorsePair">
            <div class="cgdHorse cgdHorseA">🐎</div>
            <div class="cgdHorse cgdHorseB">🐎</div>
          </div>
          <div class="cgdHarness"></div>
          <div class="cgdCart">
            <div class="cgdDriver">🏇</div>
            <div class="cgdCartBody"></div>
            <div class="cgdWheel cgdWheel1"></div>
            <div class="cgdWheel cgdWheel2"></div>
            <div class="cgdSpeedLine s1"></div>
            <div class="cgdSpeedLine s2"></div>
            <div class="cgdSpeedLine s3"></div>
          </div>
        </div>`;
  ...
  function applyZoomCompensation(){
    try{
      ...
      if(app){
        ...
        if(top){ top.style.marginBottom = "0px"; top.style.width = "100vw"; top.style.left = "0"; top.style.right = "0"; }
        if(bottom){
...
.cgdRaceScene{ position:relative; width:100%; height:100%; }
.cgdHorsePair{ position:absolute; right:92px; top:22px; display:flex; gap:10px; align-items:flex-end; }
.cgdHorse{ font-size:58px; line-height:1; filter: drop-shadow(0 6px 10px rgba(0,0,0,.18)); animation: cgdHorseBob .22s ease-in-out infinite alternate; }
.cgdHorseB{ margin-top:10px; animation-delay:.11s; }
.cgdHarness{ position:absolute; left:168px; top:72px; width:110px; height:4px; background:linear-gradient(90deg,#d4a017,#f6d25d,#d4a017); transform:rotate(-9deg); border-radius:999px; box-shadow:0 0 0 1px rgba(125,86,0,.18); }
.cgdCart{ position:absolute; left:28px; top:26px; width:170px; height:110px; }
.cgdDriver{ position:absolute; left:58px; top:-8px; font-size:42px; transform: scaleX(-1); animation:cgdDriverBob .18s ease-in-out infinite alternate; }
.cgdCartBody{ position:absolute; left:22px; top:34px; width:102px; height:44px; background:linear-gradient(180deg,#f7cc4b,#d89e12); border:3px solid #9b6808; border-radius:22px 22px 12px 12px; box-shadow: inset 0 -8px 0 rgba(125,86,0,.15), 0 6px 16px rgba(0,0,0,.12); }
.cgdCartBody::before{ content:""; position:absolute; right:-16px; top:14px; width:18px; height:10px; background:#d89e12; border:3px solid #9b6808; border-left:none; border-radius:0 10px 10px 0; }
.cgdWheel{ position:absolute; bottom:4px; width:40px; height:40px; border-radius:999px; border:5px solid #b67807; background:radial-gradient(circle at center, #fff4c6 0 18%, #f5ca52 19% 58%, #b67807 59% 100%); animation:cgdWheelSpin .36s linear infinite; box-shadow:0 4px 10px rgba(0,0,0,.14); }
.cgdWheel::before, .cgdWheel::after{ content:""; position:absolute; inset:4px 17px; background:#9b6808; border-radius:999px; }
.cgdWheel::after{ transform:rotate(90deg); transform-origin:center; }
.cgdWheel1{ left:18px; }
.cgdWheel2{ left:92px; }
.cgdSpeedLine{ position:absolute; height:3px; border-radius:999px; background:rgba(120,120,120,.45); }
.cgdSpeedLine.s1{ left:-10px; top:28px; width:26px; }
.cgdSpeedLine.s2{ left:-24px; top:52px; width:34px; }
.cgdSpeedLine.s3{ left:-6px; top:80px; width:22px; }
.cgdDust{ position:absolute; bottom:8px; width:26px; height:12px; border-radius:999px; background:rgba(196,160,92,.34); filter: blur(1px); animation:cgdDustFloat .7s linear infinite; }
.cgdDust1{ left:132px; }
.cgdDust2{ left:292px; animation-delay:.28s; }
@keyframes cgdHorseBob{ from{ transform:translateY(0) rotate(-2deg);} to{ transform:translateY(-7px) rotate(2deg);} }
@keyframes cgdDriverBob{ from{ transform:scaleX(-1) translateY(0);} to{ transform:scaleX(-1) translateY(-4px);} }
@keyframes cgdWheelSpin{ from{ transform:rotate(0deg);} to{ transform:rotate(360deg);} }
@keyframes cgdDustFloat{ from{ transform:translateX(0) scale(.9); opacity:.2;} to{ transform:translateX(-18px) scale(1.25); opacity:.0;} }
@keyframes horseRunAcross{
  0%   { transform: translateX(0); opacity: 0; }
  8%   { opacity: .98; }
  100% { transform: translateX(calc(100vw + 900px)); opacity: 0; }
}
