...
body.cgdDark .cgdBadge{ background: rgba(255,255,255,.9) !important; }
  ...
  // State (RAM)
  // =========================
  const refreshGuards = { newLeads:false, pending:false, stats:false, users:false, userLastTwo:false, queue:false, storeQueue:false };
...
async function modalBatchTransfer(){
  ...
  function stabilizeLogo(){
    ...
    const candidates = (CONFIG.LOGO_CANDIDATES && CONFIG.LOGO_CANDIDATES.length)
      ? CONFIG.LOGO_CANDIDATES.slice()
      : [CONFIG.LOGO_URL, CONFIG.LOGO_FALLBACK_URL].filter(Boolean);

    let idx = 0;
    let attempt = 0;
    const maxAttempts = Math.max(6, candidates.length * 2);
    ...
    // Se o navegador já tiver marcado erro/cache ruim, força nova tentativa.
  ...
  async function refreshUsersFast(){
    ...
    // ✅ V5: contadores separados dos “últimos leads”.
    // Antes, a etapa de últimos leads ficava dentro deste refresh e podia segurar o lock por muito tempo.
    // Resultado: uma chamada lenta do Bitrix podia impedir novos refreshes dos números por vários minutos.
    try{
      renderWho();
      ...
      // 1) Cards/gráfico por USER — agregação única por período.
      const [dayAgg, monAgg, r30Agg] = await Promise.all([
        fetchPegAggregateRange(dayS, dayE, CONFIG.COUNT_STATUS_ALLOWED),
        fetchPegAggregateRange(monS, monE, CONFIG.COUNT_STATUS_ALLOWED),
        fetchPegAggregateRange(r30S, r30E, [CONFIG.LEAD_STATUS.ATENDIDO, CONFIG.LEAD_STATUS.LEAD_CONVERTIDO_SISTEMA])
      ]);

      state.dailyByUser = dayAgg.byUser || {};
      state.monthlyByStatus = monAgg.byStatus || state.monthlyByStatus || {};

      const monthTotal = Number(monAgg.total || 0);
      state.stats = {
        day: Number(dayAgg.total || 0),
        month: monthTotal,
        pctLost: pctPart(state.monthlyByStatus[CONFIG.LEAD_STATUS.PERDIDO], monthTotal),
        pctJunk: pctPart(state.monthlyByStatus[CONFIG.LEAD_STATUS.LEAD_DESCARTADO_SISTEMA], monthTotal),
        pctConverted: pctPart(state.monthlyByStatus[CONFIG.LEAD_STATUS.LEAD_CONVERTIDO_SISTEMA], monthTotal)
      ...
      users.forEach(u=>{
        ...
        const us30 = (r30Agg.byUserStatus && r30Agg.byUserStatus[uid]) || {};
        const att30 = Number(us30[CONFIG.LEAD_STATUS.ATENDIDO] || 0);
        const conv30 = Number(us30[CONFIG.LEAD_STATUS.LEAD_CONVERTIDO_SISTEMA] || 0);
        const pct = att30 > 0 ? Math.round((conv30 / att30) * 100) : 0;
        state.userStats[u.id] = {
          ...(state.userStats[u.id]||{}),
          pulledToday: Number((dayAgg.byUser || {})[uid] || 0),
          pulledMonth: Number((monAgg.byUser || {})[uid] || 0),
          success30: { attended: att30, converted: conv30, pct }
        };
      });
      renderWho();
      setStatus(`Atualizado: ${nowBRTime()}`);
  ...
  async function start(){
    ...
    setInterval(stabilizeLogo, 60000);
