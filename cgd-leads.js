...
(function(){
  ...
  function whipCrack(ctx, t){
    ...
    for(let i=0;i<len;i++){
      const x = i / len;
      const attack = x < 0.06 ? (1 - x / 0.06) : 0;
      const burst = (Math.random() * 2 - 1) * Math.exp(-x * 16) * 1.8;
      const tail = (Math.random() * 2 - 1) * Math.exp(-Math.max(0, x - 0.05) * 8) * 0.42;
      const crack = Math.sin(i * 0.085) * Math.exp(-x * 22) * 0.55;
      data[i] = Math.max(-1, Math.min(1, burst + tail + crack + attack * 0.35));
    }
    ...
    hp.type = "highpass";
    hp.frequency.setValueAtTime(650, t);
    ...
    gain.gain.exponentialRampToValueAtTime(1.7, t + 0.008);
...
.cgdTopRight{ display:flex; gap:5px; align-items:center; flex-wrap: nowrap; justify-content: flex-end; min-width:0; overflow-x:auto; overflow-y:hidden; scrollbar-width:thin; }
...
body.cgdDark .cgdBadge{ background: rgba(255,255,255,.9) !important; }
  ...
  function renderQueueSidebar(){
    ...
    order.forEach((id, idx)=>{
      ...
      row.innerHTML = `
        <div style="display:grid; grid-template-columns:36px minmax(0,1fr); gap:10px; align-items:center; min-width:0">
