...
.cgdStoreQueueHead .sqTitle{ font-weight:950; font-size:11px; letter-spacing:.3px; white-space:nowrap; overflow:hidden; text-overflow:ellipsis; }
.cgdStoreQueueHead .storeWalk{ background: rgba(255,255,255,.96); color:#111; border-color: rgba(255,255,255,.32); padding:5px 8px; font-size:10px; border-radius:999px; }
.cgdStoreQueueBody{ padding:8px; display:flex; flex-direction:column; gap:6px; max-height: 236px; overflow:auto; }
.cgdStoreRow{
  border:1px solid rgba(255,255,255,.14);
  background: rgba(255,255,255,.10);
  border-radius: 12px;
...
.cgdStoreRow .ord{ font-size:10px; font-weight:950; opacity:.68; }
.cgdStoreRow .nm{ font-size:11px; line-height:1.1; font-weight:950; white-space:nowrap; overflow:hidden; text-overflow:ellipsis; min-width:0; }
.cgdStoreActions{ display:flex; justify-content:flex-end; gap:3px; align-items:center; }
.cgdStoreActions .cgdMiniBtn{ width:24px; min-width:24px; height:24px; padding:0; border-radius:9px; font-size:10px; border-color: rgba(255,255,255,.20); }
.cgdStorePause{ background: rgba(255,255,255,.94); color:#111; }
.cgdStorePlay{ background: rgba(120,255,170,.90); color:#111; }
.cgdStoreQueueFoot{ padding:7px 9px; border-top:1px solid rgba(255,255,255,.12); font-size:10px; font-weight:900; opacity:.78; }
...
body.cgdDark .cgdBadge{ background: rgba(255,255,255,.9) !important; }
  ...
  async function ensureStoreQueueDeal(){
    const items = await bxListAll("crm.deal.list", {
      filter: {
        CATEGORY_ID: CONFIG.STORE_QUEUE.CATEGORY_ID,
        STAGE_ID: CONFIG.STORE_QUEUE.STAGE_ID,
        "%TITLE": CONFIG.STORE_QUEUE.TITLE_KEY
      },
      order: { ID:"DESC" },
      select: ["ID","TITLE", CONFIG.STORE_QUEUE.UF_QUEUE_JSON, "DATE_MODIFY"]
    }, 5);
    ...
    const id = await bx("crm.deal.add", {
      fields: {
        CATEGORY_ID: CONFIG.STORE_QUEUE.CATEGORY_ID,
        STAGE_ID: CONFIG.STORE_QUEUE.STAGE_ID,
        TITLE: `${CONFIG.STORE_QUEUE.TITLE_KEY} FILA LOJA PRESENCIAL`,
  ...
  async function fetchStoreQueue(){
    return withStoreQueueLock(async ()=>{
      ...
      for(let attempt=0; attempt<3; attempt++){
        try{
          const deal = await ensureStoreQueueDeal();
...
async function modalBatchTransfer(){
  ...
  async function refreshUsersFast(){
    ...
    try{
      ...
      const [dayAgg, monAgg, s30Agg] = await Promise.all([
        fetchPegAggregateRange(dayS, dayE, CONFIG.COUNT_STATUS_ALLOWED),
        fetchPegAggregateRange(monS, monE, CONFIG.COUNT_STATUS_ALLOWED),
      ...
      for(let i=0;i<users.length;i+=5){
        ...
        const jobs = part.map(async u=>{
          ...
          const s30 = s30Agg.byUserStatus[uid] || {};
          const att30 = Number(s30[CONFIG.LEAD_STATUS.ATENDIDO] || 0);
          const conv30 = Number(s30[CONFIG.LEAD_STATUS.LEAD_CONVERTIDO_SISTEMA] || 0);
          const pct = (att30 > 0) ? Math.round((conv30 / att30) * 100) : 0;
  ...
  async function start(){
    ...
    await Promise.allSettled([refreshNewLeads(), refreshPendingCount(), refreshQueue(), refreshStoreQueue(), refreshUsersFast()]);
    setStatus(`Atualizado: ${nowBRTime()}`);
    renderBossPics();
    ...
    setInterval(refreshStoreQueue, CONFIG.REFRESH_QUEUE_MS);
