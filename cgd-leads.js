...
(function(){
  ...
  function leadBeep(ctx, t, freq=880, dur=0.14, gainLevel=0.24){
    ...
    g.gain.exponentialRampToValueAtTime(gainLevel * 0.78, t + Math.max(0.05, dur - 0.04));
    ...
    oscA.start(t);
    oscB.start(t);
    oscA.stop(t + dur + 0.04);
    oscB.stop(t + dur + 0.04);
  }
  ...
  function playLeadAlertFallback(){
    try{
      ...
      const fire = ()=>{
        ...
        leadBeep(ctx, base + 0.00, 880, 0.13, 0.25);
        leadBeep(ctx, base + 0.18, 988, 0.13, 0.25);
        leadBeep(ctx, base + 0.36, 1174, 0.17, 0.27);
      };
