/* cgd-leads.js — Painel de Leads (Bitrix24 Sites)
   ...
   1) Contagem DIA/MÊS voltou a funcionar:
      ...
      - Considera SOMENTE as colunas: EM ATENDIMENTO, ATENDIDO, QUALIFICADO, INTERAGIDO, LEAD DESCARTADO (JUNK), LEAD CONVERTIDO (CONVERTED - sistema).
...
(function(){
  ...
  const CONFIG = {
    ...
    // ✅ Fila manual da loja / atendimento presencial
    // Independente do PEGAR lead; usa a fila principal apenas para saber quem está disponível.
    STORE_QUEUE: {
      CATEGORY_ID: 27,
      STAGE_ID: "C27:UC_SVUYIO",
      UF_QUEUE_JSON: "UF_CRM_1779802287",
      TITLE_KEY: "__QUEUE__CGD_LOJA__",
      USER_IDS: [15, 19, 17, 23, 811, 4741, 3083, 3079, 4945]
    },
    ...
    // ✅ Logo: usa primeiro o link direto enviado do Bitrix e mantém cadeia de fallback/retry.
    // Motivo: o link do domínio do próprio portal costuma ser mais estável que o public mirror antigo.
    SYSTEM_VERSION_LABEL: "Atualizado em 03/07/2026 09:45 - GILDA 4945",
    SYSTEM_VERSION_CODE: "20260703-0945-GILDA-4945",
    LOGO_ASSET_VERSION: "20260703-0945-GILDA-4945",
    LOGO_URL: "https://b24-6iyx5y.bitrix24.com.br/docs/pub/c77325321d1ad38e8012b995a5f4e8dd/showFile?token=w1zf1f29rlm5",
    LOGO_FALLBACK_URL: "https://bitrix24public.com/b24-6iyx5y.bitrix24.com.br/docs/pub/189eb7d8a5cc26250f61ee3c26e9f997/showFile/?&token=awjcg85eqrbi",
    LOGO_CANDIDATES: [
      "https://b24-6iyx5y.bitrix24.com.br/docs/pub/c77325321d1ad38e8012b995a5f4e8dd/showFile?token=w1zf1f29rlm5",
      "https://b24-6iyx5y.bitrix24.com.br/docs/pub/c77325321d1ad38e8012b995a5f4e8dd/showFile/?token=w1zf1f29rlm5",
      "https://bitrix24public.com/b24-6iyx5y.bitrix24.com.br/docs/pub/c77325321d1ad38e8012b995a5f4e8dd/showFile?token=w1zf1f29rlm5",
      "https://bitrix24public.com/b24-6iyx5y.bitrix24.com.br/docs/pub/189eb7d8a5cc26250f61ee3c26e9f997/showFile/?&token=awjcg85eqrbi",
      "https://bitrix24public.com/b24-6iyx5y.bitrix24.com.br/docs/pub/189eb7d8a5cc26250f61ee3c26e9f997/default"
    ],

    // ✅ Logo via cadastro de empresa no CRM Bitrix.
    // Se souber o ID exato da empresa CGD no CRM, preencha COMPANY_ID.
    // Se deixar vazio, o sistema tenta localizar por TITLE_SEARCH e usa o primeiro registro com LOGO.
    COMPANY_LOGO: {
      ENABLED: true,
      COMPANY_ID: "",
      TITLE_SEARCH: "CGD",
      CACHE_MINUTES: 10,
      FORCE_REFRESH_ON_LOAD: true,
      DEBUG: true
    },

    // ✅ Timeouts menores nas métricas evitam painel preso por vários minutos quando o Bitrix fica lento.
    METRIC_COUNT_TIMEOUT_MS: 5200,
    ...
    REFRESH_STORE_QUEUE_MS: 30000,
    // ✅ Cards/contadores por user precisam atualizar rápido.
    // Os “últimos 2 leads” agora rodam separado para não travar o refresh dos números.
    REFRESH_WHO_MS: 5000,
    REFRESH_USER_LAST_TWO_MS: 45000,
    // ✅ Métricas pesadas rodam em segundo plano para não travar a atualização rápida.
    REFRESH_MONTH_METRICS_MS: 60000,
    REFRESH_SUCCESS30_MS: 90000,
    ...
    // ✅ Para “atendidos do dia/mês”: leads PEGOS no período que hoje estejam nestas etapas
    // (ATENDIDO + INTERAGIDO + QUALIFICADO + PERDIDO + LEAD DESCARTADO + LEAD CONVERTIDO)
    COUNT_STATUS_ALLOWED: [
      "UC_JT9G60",  // ATENDIDO
      "UC_0NFA3H",  // QUALIFICADO
      "UC_46FEKJ",  // INTERAGIDO
      "UC_5IMTI4",  // PERDIDO / SEM RESPOSTA
      "JUNK",       // LEAD DESCARTADO (CRM Leads / sistema)
      "CONVERTED"   // LEAD CONVERTIDO (CRM Leads / sistema)
    ],

    // ✅ Limite alto para agregar métricas em 1 busca por período, em vez de várias buscas por USER
    LIMIT_STATS_AGGREGATE: 10000,
...
.cgdQueueBody{ padding: 8px; overflow:auto; min-height: 0; display:flex; flex-direction: column; gap: 6px; }
.cgdQueueRowItem{
  border: 1px solid var(--border);
  border-radius: 14px;
...
.cgdQueueArrows{ display:flex; gap:4px; align-items:center; justify-content:flex-end; align-self:center; min-width:64px; flex:0 0 64px; }
.cgdQueueArrows .cgdMiniBtn{ min-width: 28px; width: 28px; height: 24px; padding: 0; line-height: 1; display:inline-flex; align-items:center; justify-content:center; flex:0 0 32px; font-size:11px; }

.cgdStoreQueue{
  margin: 7px 8px 8px;
  border: 1px solid rgba(30,40,70,.16);
  border-radius: 14px;
  background: linear-gradient(180deg, rgba(20,24,34,.92), rgba(45,48,58,.88));
  color:#fff;
  overflow:hidden;
  box-shadow: 0 10px 24px rgba(10,14,25,.18);
}
.cgdStoreQueueHead{
  display:flex;
  align-items:center;
...
.cgdStoreQueueHead .sqTitle{ font-weight:950; font-size:10px; letter-spacing:.3px; white-space:nowrap; overflow:hidden; text-overflow:ellipsis; }
.cgdStoreQueueHead .storeWalk{ background: rgba(255,255,255,.96); color:#111; border-color: rgba(255,255,255,.32); padding:3px 7px; font-size:9px; border-radius:999px; }
.cgdStoreQueueBody{ padding:5px 6px; display:flex; flex-direction:column; gap:3px; max-height:none; overflow:visible; }
.cgdStoreRow{
  border:1px solid rgba(255,255,255,.14);
  background: rgba(255,255,255,.10);
  border-radius: 9px;
...
.cgdStoreRow.paused{ background: rgba(255,210,80,.20); border-color: rgba(255,210,80,.40); color: rgba(255,255,255,.72); }
.cgdStoreRow .ord{ font-size:9px; font-weight:950; opacity:.68; }
.cgdStoreRow .nm{ font-size:10px; line-height:1.1; font-weight:950; white-space:nowrap; overflow:hidden; text-overflow:ellipsis; min-width:0; }
.cgdStoreActions{ display:flex; justify-content:flex-end; gap:2px; align-items:center; }
.cgdStoreActions .cgdMiniBtn{ width:20px; min-width:20px; height:19px; padding:0; border-radius:7px; font-size:9px; border-color: rgba(255,255,255,.20); }
.cgdStorePause{ background: rgba(255,255,255,.94); color:#111; }
.cgdStorePlay{ background: rgba(120,255,170,.90); color:#111; }
.cgdStoreQueueFoot{ padding:4px 7px; border-top:1px solid rgba(255,255,255,.12); font-size:10px; font-weight:900; opacity:.78; }
...
#listWho.cgdWhoGrid{ display:grid !important; grid-template-columns: repeat(auto-fit, minmax(285px, 1fr)); gap: 10px; align-content:start; }
@media (max-width: 1100px){ #listWho.cgdWhoGrid{ grid-template-columns: repeat(auto-fit, minmax(270px, 1fr)); } }
.cgdDailyChart{
  margin: 0 10px 10px;
  border: 1px solid var(--border);
  border-radius: 16px;
...
.cgdDailyChartTitle{ font-weight:950; font-size:12px; letter-spacing:.2px; margin-bottom:8px; text-transform:uppercase; }
.cgdDailyBars{ display:flex; align-items:flex-end; gap:7px; min-height:120px; overflow-x:auto; padding:4px 2px 0; }
.cgdDailyBarWrap{ min-width:42px; flex:1 0 42px; display:flex; flex-direction:column; align-items:center; justify-content:flex-end; gap:5px; }
.cgdDailyBarValue{ font-size:10px; font-weight:950; line-height:1; }
.cgdDailyBar{ width:100%; max-width:38px; min-height:4px; border-radius:9px 9px 3px 3px; background: rgba(17,24,39,.88); }
.cgdDailyBarName{ width:100%; font-size:9px; line-height:1.05; font-weight:950; text-align:center; white-space:nowrap; overflow:hidden; text-overflow:ellipsis; }
...
.cgdBottom .bLeft{ display:flex; align-items:center; gap:10px; min-width: 520px; flex:0 0 auto; }
.cgdLossStats{ display:flex; gap:6px; align-items:stretch; flex:0 0 auto; }
.cgdLossBox{
  min-width:92px;
  border:1px solid rgba(255,255,255,.16);
  border-radius:12px;
...
.cgdLossBox .lbl{ font-size:8px; line-height:1.05; font-weight:950; opacity:.75; text-transform:uppercase; white-space:nowrap; }
.cgdLossBox .val{ font-size:13px; line-height:1.1; font-weight:950; margin-top:2px; }
...
body.cgdDark .cgdBadge{ background: rgba(255,255,255,.9) !important; }
  ...
  // State (RAM)
  // =========================
  const refreshGuards = { newLeads:false, pending:false, stats:false, users:false, monthMetrics:false, success30:false, userLastTwo:false, queue:false, storeQueue:false };
  ...
  const state = {
    ...
    stats: { day:0, month:0, pctLost:0, pctJunk:0, pctConverted:0 },
    dailyByUser: {},
    monthlyByStatus: {},
    userStats: {},
    ...
    storeQueue: { order:[], pausedStoreOnly:[], updatedAt:0, dealId:null },
    storeQueueLocalTouchTs: 0,
  ...
  function applyColumnZoom(){
    const pairs = [
      ...
      ["storeQueueBody", state.columnZoom.queue],
    ];
      ...
      <div id="cgdApp">
        <div class="cgdTop">
          <div class="cgdTopLeft">
            <img class="cgdLogo" id="cgdLogo" src="${esc(CONFIG.LOGO_URL)}" alt="CGD" referrerpolicy="no-referrer" loading="eager" decoding="async" />
            <div class="cgdTitle">PAINEL DE LEADS • CGD CORRETORA</div>
            <div class="cgdVersion" id="cgdVersion">${esc(CONFIG.SYSTEM_VERSION_LABEL || "")}</div>
          </div>
        ...
        <div class="cgdLayout">
          <div class="cgdGrid">
            ...
            <section class="cgdCol" id="colWho">
              ...
              <div class="cgdDailyChart" id="dailyUserChart">
                <div class="cgdDailyChartTitle">Atendidos no dia por user</div>
                <div class="cgdDailyBars" id="dailyUserBars">
                  <div style="opacity:.7;font-weight:900;font-size:11px">Carregando gráfico…</div>
                </div>
              </div>
            </section>
          </div>
          ...
          <aside class="cgdQueueSide" id="queueSide">
            ...
            <div class="cgdStoreQueue" id="storeQueueBox">
              <div class="cgdStoreQueueHead">
                <div class="sqTitle">LOJA • PRESENCIAL</div>
                <button class="cgdMiniBtn storeWalk" id="btnStoreQueueWalk">ANDAR</button>
              </div>
              <div class="cgdStoreQueueBody cgdZoomPane" id="storeQueueBody">
                <div style="opacity:.7;font-weight:900">Carregando fila presencial…</div>
              </div>
              <div class="cgdStoreQueueFoot">
                <span id="storeQueueHint">Manual • sem PEGAR lead</span>
              </div>
            </div>
          </aside>
        </div>
        ...
        <div class="cgdBottom">
          <div class="bLeft">
            ...
            <div class="cgdLossStats" id="lossStats">
              <div class="cgdLossBox"><div class="lbl">Sem resposta</div><div class="val" id="pctLost">0%</div></div>
              <div class="cgdLossBox"><div class="lbl">Desistentes</div><div class="val" id="pctJunk">0%</div></div>
              <div class="cgdLossBox"><div class="lbl">Convertidos</div><div class="val" id="pctConverted">0%</div></div>
            </div>
            <div>
  ...
  async function ensureQueueDeal(){
    const items = await bxListAll("crm.deal.list", {
      ...
      order: { ID:"DESC" },
      select: ["ID","TITLE", CONFIG.QUEUE.UF_QUEUE_JSON, CONFIG.STORE_QUEUE.UF_QUEUE_JSON, "DATE_MODIFY"]
    }, 5);
    ...
    const id = await bx("crm.deal.add", {
      fields: {
        ...
        [CONFIG.STORE_QUEUE.UF_QUEUE_JSON]: JSON.stringify({ v:1, order: defaultStoreOrder(), pausedStoreOnly:[], updatedAt: Date.now() })
      }
    });
  ...
  function activeUserIdSet(){
    return new Set(CONFIG.USERS.map(u=>String(u.id)));
  }

  function sanitizeUserIdList(list){
    const valid = activeUserIdSet();
    const seen = new Set();
  ...
  function parseQueue(json){
    try{
      ...
      const order = sanitizeUserIdList(o.order);
      const hiddenUsers = sanitizeUserIdList(o.hiddenUsers);
    ...
    return withQueueLock(async ()=>{
      ...
      for(let attempt=0; attempt<3; attempt++){
        try{
          ...
          const storeRaw = deal && deal[CONFIG.STORE_QUEUE.UF_QUEUE_JSON];
          return { dealId: String(deal.ID), storeRaw, storeParsed: parseStoreQueue(storeRaw), ...parseQueue(raw) };
        }catch(err){
          lastErr = err;
  ...
  async function saveQueue(dealId, payload){
    return withQueueLock(async ()=>{
      const next = {
        v: 1,
        order: sanitizeUserIdList(payload.order),
        hiddenUsers: sanitizeUserIdList(payload.hiddenUsers),
        updatedAt: Date.now()
      };
  ...
  async function persistQueueOrder(nextOrder){
    ...
    renderQueueSidebar();
    renderStoreQueueSidebar();
    enqueueOp("saveQueueOrder", async ()=>{ await saveQueue(q.dealId, { order: nextOrder, hiddenUsers: q.hiddenUsers||[] }); });
    flushOps();
  }

  function storeUserIdSet(){
    return new Set((CONFIG.STORE_QUEUE.USER_IDS || []).map(String));
  }

  function defaultStoreOrder(){
    return (CONFIG.STORE_QUEUE.USER_IDS || []).map(String);
  }

  function sanitizeStoreUserIdList(list){
    const valid = storeUserIdSet();
    const seen = new Set();
    return (Array.isArray(list) ? list : [])
      .map(String)
      .filter(id=> valid.has(id) && !seen.has(id) && (seen.add(id), true));
  ...
  function normalizeStoreOrder(order){
    const clean = sanitizeStoreUserIdList(order);
    const present = new Set(clean);
    const missing = defaultStoreOrder().filter(id=> !present.has(id));
    return clean.concat(missing);
  }

  async function ensureStoreQueueDeal(){
    // Usa o MESMO registro técnico da fila principal para evitar consultas extras e lentidão.
    // A fila presencial fica separada apenas por campo JSON (CONFIG.STORE_QUEUE.UF_QUEUE_JSON).
    const q = state.queue.dealId ? { ID: state.queue.dealId } : await ensureQueueDeal();
    return q;
  }

  function parseStoreQueue(json){
    try{
      const o = JSON.parse(json || "{}");
      const order = normalizeStoreOrder(o.order);
      const pausedStoreOnly = sanitizeStoreUserIdList(o.pausedStoreOnly);
      const updatedAt = +o.updatedAt || 0;
      return { order, pausedStoreOnly, updatedAt };
    }catch(_){
      return { order: defaultStoreOrder(), pausedStoreOnly:[], updatedAt:0 };
    }
  }

  let storeQueueBusy = false;
  async function withStoreQueueLock(fn){
    for(let i=0; i<20 && storeQueueBusy; i++) await sleep(60);
    storeQueueBusy = true;
    try{ return await fn(); }
    finally{ storeQueueBusy = false; }
  }

  async function fetchStoreQueue(){
    return withStoreQueueLock(async ()=>{
      let lastErr = null;
      for(let attempt=0; attempt<3; attempt++){
        try{
          let deal = null;
          if(state.queue.dealId){
            deal = await bx("crm.deal.get", { id: String(state.queue.dealId) });
          }else{
            deal = await ensureQueueDeal();
          }
          const raw = deal && deal[CONFIG.STORE_QUEUE.UF_QUEUE_JSON];
          return { dealId: String(deal.ID), ...parseStoreQueue(raw) };
        }catch(err){
          lastErr = err;
          await sleep(200 + attempt*350);
        }
      }
      throw lastErr || new Error("Falha ao carregar fila presencial");
    });
  }

  async function saveStoreQueue(dealId, payload){
    return withStoreQueueLock(async ()=>{
      const next = {
        v: 1,
        order: normalizeStoreOrder(payload.order),
        pausedStoreOnly: sanitizeStoreUserIdList(payload.pausedStoreOnly),
        updatedAt: Date.now()
      };
      ...
      for(let attempt=0; attempt<3; attempt++){
        try{
          await bx("crm.deal.update", {
            id: String(dealId),
            fields: { [CONFIG.STORE_QUEUE.UF_QUEUE_JSON]: JSON.stringify(next) }
          });
          return;
        }catch(err){
          lastErr = err;
      ...
      throw lastErr || new Error("Falha ao salvar fila presencial");
    });
  }

  function mainQueueActiveStoreSet(){
    const storeValid = storeUserIdSet();
    return new Set((state.queue.order || []).map(String).filter(id=> storeValid.has(id)));
  }

  function activeStoreOrder(){
    const mainActive = mainQueueActiveStoreSet();
    const paused = new Set((state.storeQueue.pausedStoreOnly || []).map(String));
    return normalizeStoreOrder(state.storeQueue.order).filter(id=> mainActive.has(id) && !paused.has(id));
  }

  function renderStoreQueueSidebar(){
    const body = $("#storeQueueBody");
    if(!body) return;
    body.innerHTML = "";

    const order = normalizeStoreOrder(state.storeQueue.order);
    const mainActive = mainQueueActiveStoreSet();
    const paused = new Set((state.storeQueue.pausedStoreOnly || []).map(String));
    const visible = order.filter(id=> mainActive.has(id));

    const hint = $("#storeQueueHint");
    if(hint){
      const activeCount = visible.filter(id=> !paused.has(id)).length;
      hint.textContent = `${activeCount} ativa(s) • vinculada à fila principal`;
    }
    ...
    if(visible.length === 0){
      ...
      d.textContent = "Nenhuma usuária disponível na fila principal.";
      body.appendChild(d);
      return;
    }
    ...
    visible.forEach((id)=>{
      ...
      const row = document.createElement("div");
      row.className = `cgdStoreRow${isPaused ? " paused" : ""}`;
      row.innerHTML = `
        <div class="ord">#${idx+1}</div>
        <div class="nm" title="${esc(u ? u.name : ("USER "+id))}">${esc(u ? u.name : ("USER "+id))}</div>
        <div class="cgdStoreActions">
          <button class="cgdMiniBtn" data-sq-up="${esc(id)}">↑</button>
          <button class="cgdMiniBtn" data-sq-down="${esc(id)}">↓</button>
          ${isPaused
            ? `<button class="cgdMiniBtn cgdStorePlay" title="Reativar só na fila presencial" data-sq-play="${esc(id)}">▶</button>`
            : `<button class="cgdMiniBtn cgdStorePause" title="Pausar só na fila presencial" data-sq-pause="${esc(id)}">Ⅱ</button>`}
        </div>
      `;
      body.appendChild(row);
    });
  ...
  function moveStoreQueueLocal(userId, dir){
    const id = String(userId);
    const arr = normalizeStoreOrder(state.storeQueue.order);
    const i = arr.indexOf(id);
    if(i < 0) return arr;
    if(dir==="up" && i>0){
      const t = arr[i-1]; arr[i-1]=arr[i]; arr[i]=t;
  ...
  async function persistStoreQueue(nextOrder, nextPaused){
    const q = state.storeQueue.dealId ? state.storeQueue : await fetchStoreQueue();
    state.storeQueueLocalTouchTs = Date.now();
    state.storeQueue = { ...state.storeQueue, ...q, order: normalizeStoreOrder(nextOrder), pausedStoreOnly: sanitizeStoreUserIdList(nextPaused) };
    renderStoreQueueSidebar();
    enqueueOp("saveStoreQueue", async ()=>{
      await saveStoreQueue(q.dealId, { order: state.storeQueue.order, pausedStoreOnly: state.storeQueue.pausedStoreOnly });
    });
    flushOps();
  }
  ...
  // Usa DATA PEGAR, pois a métrica pedida é “leads pegos no dia”.
  function countFilterBase(startISO, endISO, statuses){
    return {
      [">=" + CONFIG.UF_DATA_PEGAR]: startISO,
      ["<"  + CONFIG.UF_DATA_PEGAR]: endISO,
      "STATUS_ID": (statuses || CONFIG.COUNT_STATUS_ALLOWED).slice()
    };
  }

  async function fetchPegCountRangeAll(startISO, endISO, statuses){
    const data = await bxRaw("crm.lead.list", {
      filter: countFilterBase(startISO, endISO, statuses),
      order: { ID: "DESC" },
      select: ["ID"],
      start: 0
    }, { timeoutMs: CONFIG.METRIC_COUNT_TIMEOUT_MS || 8000 });
    const total = Number(data && data.total);
    if(Number.isFinite(total)) return total;
    const items = Array.isArray(data?.result) ? data.result : [];
    return items.length;
  ...
  async function fetchPegCountRangeUser(userId, startISO, endISO, statuses){
    const data = await bxRaw("crm.lead.list", {
      filter: {
        ...countFilterBase(startISO, endISO, statuses),
  ...
  async function fetchUserCountMapRange(startISO, endISO, statuses, users=CONFIG.USERS, concurrency=4){
    const byUser = {};
    await runPool(users, concurrency, async (u)=>{
      try{
        byUser[String(u.id)] = await fetchPegCountRangeUser(u.id, startISO, endISO, statuses);
      }catch(err){
        console.warn("user count failed", u && u.name, err);
        byUser[String(u.id)] = Number(byUser[String(u.id)] || 0);
      }
    });
    return byUser;
  }

  // ✅ Agrega por USER em uma única varredura do período.
  // Isso evita dezenas de chamadas separadas ao Bitrix e acelera as métricas do dia.
  async function fetchPegAggregateRange(startISO, endISO, statuses){
    const rows = await bxListAll("crm.lead.list", {
      filter: countFilterBase(startISO, endISO, statuses),
      order: { ID: "DESC" },
      select: ["ID", "ASSIGNED_BY_ID", "STATUS_ID"]
    }, CONFIG.LIMIT_STATS_AGGREGATE);

    const byUser = {};
    const byStatus = {};
    const byUserStatus = {};
    (rows || []).forEach(r=>{
      const uid = String(r.ASSIGNED_BY_ID || "");
      const st = String(r.STATUS_ID || "");
      if(st) byStatus[st] = (byStatus[st] || 0) + 1;
      if(!uid) return;
      byUser[uid] = (byUser[uid] || 0) + 1;
      if(!byUserStatus[uid]) byUserStatus[uid] = {};
      byUserStatus[uid][st] = (byUserStatus[uid][st] || 0) + 1;
    });

    return { total: (rows || []).length, byUser, byStatus, byUserStatus };
  }
  ...
  function renderStats(stats){
    ...
    const pLost = $("#pctLost");
    const pJunk = $("#pctJunk");
    const pConv = $("#pctConverted");
    if(pLost) pLost.textContent = `${stats.pctLost||0}%`;
    if(pJunk) pJunk.textContent = `${stats.pctJunk||0}%`;
    if(pConv) pConv.textContent = `${stats.pctConverted||0}%`;
    renderDailyUserChart();
  }

  function safeHiddenUsers(){
    try{ return sanitizeUserIdList(state.queue && state.queue.hiddenUsers); }catch(_){ return []; }
  }

  function renderDailyUserChart(){
    const box = $("#dailyUserBars");
    if(!box) return;
    const byUser = state.dailyByUser || {};
    const hiddenSet = new Set(safeHiddenUsers());
    const rows = CONFIG.USERS
      .filter(u => !hiddenSet.has(String(u.id)))
      .map(u=>({ ...u, n:Number(byUser[String(u.id)] || 0) }));
    const max = Math.max(1, ...rows.map(r=>r.n));
    box.innerHTML = "";
    if(rows.length === 0){
      box.innerHTML = `<div style="opacity:.75;font-weight:900;font-size:11px">Nenhuma usuária visível no gráfico.</div>`;
      return;
    }
    rows.forEach(r=>{
      const h = Math.max(4, Math.round((r.n / max) * 96));
      const wrap = document.createElement("div");
      wrap.className = "cgdDailyBarWrap";
      wrap.title = `${r.name}: ${r.n}`;
      wrap.innerHTML = `
        <div class="cgdDailyBarValue">${esc(r.n)}</div>
        <div class="cgdDailyBar" style="height:${esc(h)}px"></div>
        <div class="cgdDailyBarName">${esc(r.name.split(" ")[0])}</div>
      `;
  ...
  function renderWho(){
    ...
    let ordered = [];
    try{
      ordered = computeUserOrder();
    }catch(err){
      console.warn("renderWho failed before render", err);
      ordered = CONFIG.USERS.slice();
    }

    const frag = document.createDocumentFragment();
    ordered.forEach(u=>{
      const us = state.userStats[u.id] || { pulledToday:0, pulledMonth:0, lastTwo:[], success30:{attended:0, converted:0, pct:0} };
      const l1 = us.lastTwo && us.lastTwo[0];
      const l2 = us.lastTwo && us.lastTwo[1];
        ...
        <div class="cgdUserLine">
          ...
          <div style="width:100%; min-width:0">
            <div style="display:flex; align-items:flex-start; justify-content:space-between; gap:8px">
              <div class="cgdUserName" title="${esc(u.name)}">${esc(u.name)}</div>
  ...
  async function fetchUserLastTwoFast(userId){
    ...
    const lastTwo = (last||[]).filter(x=>{
      ...
      return st===CONFIG.LEAD_STATUS.EM_ATENDIMENTO || st===CONFIG.LEAD_STATUS.QUALIFICADO || st===CONFIG.LEAD_STATUS.INTERAGIDO || st===CONFIG.LEAD_STATUS.ATENDIDO;
    }).slice(0,2);
  ...
  async function fetchUserStatsFull(userId){
    ...
    const [pulledToday, pulledMonth, att30, conv30] = await Promise.all([
...
async function modalBatchTransfer(){
  ...
  // =========================
  // Logo do cadastro da empresa no Bitrix
  // =========================
  function absBitrixUrl(url){
    if(!url) return "";
    const s = String(url);
    if(/^https?:\/\//i.test(s)) return s;
    if(s.startsWith("//")) return "https:" + s;
    const base = CONFIG.WEBHOOK.replace(/\/rest\/.*$/i, "");
    if(s.startsWith("/")) return base + s;
    return base + "/" + s;
  }

  function extractLogoUrlFromValue(v){
    if(!v) return "";
    if(typeof v === "string") return absBitrixUrl(v);
    if(Array.isArray(v)){
      for(const x of v){
        const u = extractLogoUrlFromValue(x);
        if(u) return u;
      }
      return "";
    }
    if(typeof v === "object"){
      const keys = ["downloadUrl","download_url","url","URL","showUrl","show_url","src","SRC"];
      for(const k of keys){
        if(v[k]) return absBitrixUrl(v[k]);
      }
      if(v.fileData && v.fileData.downloadUrl) return absBitrixUrl(v.fileData.downloadUrl);
    }
    return "";
  }

  function logoCacheKey(){ return `cgdCompanyLogoUrl:${CONFIG.SYSTEM_VERSION_CODE || "1"}`; }

  function logoDebug(...args){
    try{ if(CONFIG.COMPANY_LOGO && CONFIG.COMPANY_LOGO.DEBUG) console.log("[CGD LOGO]", ...args); }catch(_){}
  }

  function looksLikeNumericId(v){ return /^\d+$/.test(String(v||"").trim()); }

  function extractLogoFileIdFromValue(v){
    if(!v) return "";
    if(typeof v === "number") return String(v);
    if(typeof v === "string") return looksLikeNumericId(v) ? String(v).trim() : "";
    if(Array.isArray(v)){
      for(const x of v){
        const id = extractLogoFileIdFromValue(x);
        if(id) return id;
      }
      return "";
    }
    if(typeof v === "object"){
      const keys = ["id","ID","fileId","file_id","FILE_ID","value","VALUE"];
      for(const k of keys){
        if(looksLikeNumericId(v[k])) return String(v[k]).trim();
      }
      if(v.fileData){
        const id = extractLogoFileIdFromValue(v.fileData);
        if(id) return id;
      }
    }
    return "";
  ...
  async function tryResolveLogoByDiskFileId(fileId){
    if(!fileId) return "";
    try{
      const f = await bx("disk.file.get", { id: String(fileId) }, { timeoutMs: 6500 });
      logoDebug("disk.file.get", fileId, f);
      return extractLogoUrlFromValue(f) || extractLogoUrlFromValue(f && f.DOWNLOAD_URL) || "";
    }catch(err){
      logoDebug("disk.file.get failed", fileId, err);
      return "";
    }
  }

  async function resolveLogoFromCompany(company){
    if(!company) return "";
    logoDebug("empresa candidata", { ID: company.ID, TITLE: company.TITLE, LOGO: company.LOGO });

    let url = extractLogoUrlFromValue(company.LOGO);
    if(url) return url;

    const fileId = extractLogoFileIdFromValue(company.LOGO);
    if(fileId){
      url = await tryResolveLogoByDiskFileId(fileId);
      if(url) return url;
    }

    // Alguns Bitrix retornam o arquivo em campos derivados/UF personalizados, não exatamente em LOGO.
    // Mantém busca tolerante, mas só aceita campos com nome relacionado a logo/imagem/foto.
    for(const [k,v] of Object.entries(company || {})){
      if(!/(logo|imagem|image|foto|photo|picture)/i.test(k)) continue;
      url = extractLogoUrlFromValue(v);
      if(url) return url;
      const id = extractLogoFileIdFromValue(v);
      if(id){
        url = await tryResolveLogoByDiskFileId(id);
        if(url) return url;
      }
    }
  ...
  async function fetchCompanyLogoUrl(){
    const cfg = CONFIG.COMPANY_LOGO || {};
    if(!cfg.ENABLED) return "";

    if(!cfg.FORCE_REFRESH_ON_LOAD){
      try{
        const cachedRaw = localStorage.getItem(logoCacheKey());
        if(cachedRaw){
          const cached = JSON.parse(cachedRaw);
          const age = Date.now() - Number(cached.ts || 0);
          const maxAge = Math.max(2, Number(cfg.CACHE_MINUTES || 10)) * 60 * 1000;
          if(cached.url && age < maxAge){
            logoDebug("usando cache", cached.url);
            return cached.url;
          }
        }
      }catch(_){}
    ...
    if(cfg.COMPANY_ID){
      try{
        company = await bx("crm.company.get", { id: String(cfg.COMPANY_ID) }, { timeoutMs: 6500 });
        const url = await resolveLogoFromCompany(company);
        if(url){
          try{ localStorage.setItem(logoCacheKey(), JSON.stringify({ url, ts: Date.now(), source: "crm.company.get", companyId: company && company.ID })); }catch(_){}
          return url;
        }
      }catch(err){
        logoDebug("crm.company.get failed", err);
      }
    }

    try{
      const rows = await bx("crm.company.list", {
        filter: { "%TITLE": cfg.TITLE_SEARCH || "CGD" },
        order: { ID: "ASC" },
        select: ["ID", "TITLE", "LOGO", "UF_*"]
      }, { timeoutMs: 6500 });
      const arr = Array.isArray(rows) ? rows : [];
      logoDebug("crm.company.list retornou", arr.map(c=>({ID:c.ID, TITLE:c.TITLE, LOGO:c.LOGO})));

      for(const c of arr){
        const url = await resolveLogoFromCompany(c);
        if(url){
          try{ localStorage.setItem(logoCacheKey(), JSON.stringify({ url, ts: Date.now(), source: "crm.company.list", companyId: c && c.ID })); }catch(_){}
          return url;
        }
      }
    }catch(err){
      logoDebug("crm.company.list failed", err);
    }
  ...
  // Logo estável
  // =========================
  function logoUrlWithVersion(url, attempt){
    if(!url) return "";
    const sep = String(url).includes("?") ? "&" : "?";
    return `${url}${sep}v=${encodeURIComponent(CONFIG.LOGO_ASSET_VERSION || "1")}&try=${attempt || 0}`;
  }

  async function stabilizeLogo(){
    const img = document.getElementById("cgdLogo");
    if(!img) return;

    const baseCandidates = (CONFIG.LOGO_CANDIDATES && CONFIG.LOGO_CANDIDATES.length)
      ? CONFIG.LOGO_CANDIDATES.slice()
      : [CONFIG.LOGO_URL, CONFIG.LOGO_FALLBACK_URL].filter(Boolean);

    let companyUrl = "";
    try{ companyUrl = await fetchCompanyLogoUrl(); }catch(_){}

    const candidates = [];
    if(companyUrl) candidates.push(companyUrl);
    baseCandidates.forEach(u=>{ if(u && !candidates.includes(u)) candidates.push(u); });

    let idx = 0;
    let attempt = 0;
    const maxAttempts = Math.max(8, candidates.length * 2);

    const useNext = ()=>{
      if(!candidates.length || attempt >= maxAttempts){
      ...
      img.onload = ()=>{
        if(img.naturalWidth > 0 && img.naturalHeight > 0){
          ...
          img.dataset.logoSource = url === companyUrl ? "crm-company" : "fallback";
          img.title = url === companyUrl ? "Logo carregada do CRM Bitrix" : "Logo carregada por fallback";
          logoDebug("logo carregada", img.dataset.logoSource, url);
          img.style.display = "block";
        }else{
          setTimeout(useNext, 250);
        }
      };
      img.onerror = ()=> setTimeout(useNext, Math.min(1500, 250 * attempt));
      img.src = logoUrlWithVersion(url, attempt);
    };

    if(img.complete && (!img.naturalWidth || !img.naturalHeight)) useNext();
    else {
      img.onerror = ()=> setTimeout(useNext, 250);
      img.onload = ()=>{ img.dataset.loaded = "1"; };
      img.src = logoUrlWithVersion(candidates[0], 0);
    }
  }
  ...
  async function refreshStats(){
    ...
    try{
      ...
      const [dayAgg, monAgg] = await Promise.all([
        fetchPegAggregateRange(dayS, dayE, CONFIG.COUNT_STATUS_ALLOWED),
        fetchPegAggregateRange(monS, monE, CONFIG.COUNT_STATUS_ALLOWED)
      ]);

      state.dailyByUser = dayAgg.byUser || {};
      state.monthlyByStatus = monAgg.byStatus || {};
      const monthTotal = Number(monAgg.total || 0);
      state.stats = {
        day: dayAgg.total || 0,
        month: monthTotal,
        pctLost: pctPart(state.monthlyByStatus[CONFIG.LEAD_STATUS.PERDIDO], monthTotal),
        pctJunk: pctPart(state.monthlyByStatus[CONFIG.LEAD_STATUS.LEAD_DESCARTADO_SISTEMA], monthTotal),
        pctConverted: pctPart(state.monthlyByStatus[CONFIG.LEAD_STATUS.LEAD_CONVERTIDO_SISTEMA], monthTotal)
      };
  ...
  async function refreshUsersFast(){
    ...
    // ✅ V7: métricas rápidas progressivas.
    // Não varre todos os leads do dia. Faz contagem por usuária usando o total do Bitrix
    // e vai renderizando conforme cada resposta chega. Assim o painel não fica “em branco”
    // aguardando uma varredura grande ou uma chamada lenta.
    try{
      const { startISO: dayS, endISO: dayE } = dayRangePortal();
      setStatus(`Carregando métricas rápidas… ${nowBRTime()}`);
      renderWho();

      let totalByUsers = 0;
      let done = 0;

      const totalPromise = fetchPegCountRangeAll(dayS, dayE, CONFIG.COUNT_STATUS_ALLOWED)
        .then(total => {
          state.stats = { ...(state.stats || {}), day: Number(total || 0) };
          renderStats(state.stats);
        })
        .catch(err => console.warn("day total failed", err));

      await runPool(CONFIG.USERS.slice(), 6, async (u)=>{
        let n = Number(state.userStats[u.id]?.pulledToday || 0);
        try{
          n = await fetchPegCountRangeUser(u.id, dayS, dayE, CONFIG.COUNT_STATUS_ALLOWED);
        }catch(err){
          console.warn("fast user count failed", u && u.name, err);
        }

        state.dailyByUser[String(u.id)] = Number(n || 0);
        state.userStats[u.id] = {
          ...(state.userStats[u.id]||{}),
          pulledToday: Number(n || 0)
        };

        totalByUsers += Number(n || 0);
        done++;

        // Mostra números parciais rapidamente, sem esperar todas as usuárias.
        if(done === 1 || done % 2 === 0 || done === CONFIG.USERS.length){
          if(!Number(state.stats?.day)) state.stats = { ...(state.stats || {}), day: totalByUsers };
          renderStats(state.stats);
          renderWho();
          setStatus(`Métricas rápidas: ${done}/${CONFIG.USERS.length} usuárias • ${nowBRTime()}`);
        }
      });

      await Promise.race([totalPromise, sleep(1200)]);
      renderWho();
      setStatus(`Métricas rápidas atualizadas: ${nowBRTime()}`);
    }catch(err){
      console.warn("user fast stats failed", err);
      renderWho();
    }finally{
  ...
  async function refreshMonthMetrics(){
    if(refreshGuards.monthMetrics) return;
    refreshGuards.monthMetrics = true;

    // ✅ Métricas do mês por USER sem varrer 10.000 leads: usa contagens por usuária/status.
    // Pode demorar alguns segundos, mas não bloqueia a métrica rápida do dia.
    try{
      const { startISO: monS, endISO: monE } = monthRangePortal();

      const [byUser, total, lost, junk, converted] = await Promise.all([
        fetchUserCountMapRange(monS, monE, CONFIG.COUNT_STATUS_ALLOWED, CONFIG.USERS, 4),
        fetchPegCountRangeAll(monS, monE, CONFIG.COUNT_STATUS_ALLOWED),
        fetchPegCountRangeAll(monS, monE, [CONFIG.LEAD_STATUS.PERDIDO]),
        fetchPegCountRangeAll(monS, monE, [CONFIG.LEAD_STATUS.LEAD_DESCARTADO_SISTEMA]),
        fetchPegCountRangeAll(monS, monE, [CONFIG.LEAD_STATUS.LEAD_CONVERTIDO_SISTEMA])
      ]);

      state.monthlyByStatus = {
        ...(state.monthlyByStatus || {}),
        [CONFIG.LEAD_STATUS.PERDIDO]: Number(lost || 0),
        [CONFIG.LEAD_STATUS.LEAD_DESCARTADO_SISTEMA]: Number(junk || 0),
        [CONFIG.LEAD_STATUS.LEAD_CONVERTIDO_SISTEMA]: Number(converted || 0)
      };
      ...
      CONFIG.USERS.forEach(u=>{
        ...
        state.userStats[u.id] = {
          ...(state.userStats[u.id]||{}),
          pulledMonth: Number((byUser || {})[uid] || 0)
        };
      });
      renderWho();
      setStatus(`Métricas do mês atualizadas: ${nowBRTime()}`);
    }catch(err){
      console.warn("month metrics failed", err);
    }finally{
      refreshGuards.monthMetrics = false;
    }
  }

  async function refreshSuccess30Metrics(){
    if(refreshGuards.success30) return;
    refreshGuards.success30 = true;

    try{
      const { startISO: r30S, endISO: r30E } = last30DaysRangePortal();
      const users = CONFIG.USERS.slice();
      await runPool(users, 3, async (u)=>{
        const [att30, conv30] = await Promise.all([
          fetchPegCountRangeUser(u.id, r30S, r30E, [CONFIG.LEAD_STATUS.ATENDIDO]),
          fetchPegCountRangeUser(u.id, r30S, r30E, [CONFIG.LEAD_STATUS.LEAD_CONVERTIDO_SISTEMA])
        ]);
        const pct = Number(att30) > 0 ? Math.round((Number(conv30 || 0) / Number(att30 || 0)) * 100) : 0;
        state.userStats[u.id] = {
          ...(state.userStats[u.id]||{}),
          success30: { attended: Number(att30 || 0), converted: Number(conv30 || 0), pct }
        };
      });
      renderWho();
    }catch(err){
      console.warn("success30 metrics failed", err);
    }finally{
      refreshGuards.success30 = false;
    }
  }

  async function refreshUsersLastTwo(){
    if(refreshGuards.userLastTwo) return;
    refreshGuards.userLastTwo = true;
    ...
    try{
      ...
      await runPool(users, 2, async (u)=>{
        try{
          ...
          console.warn("last two failed", u && u.name, err);
        }
        lastDone++;
        if(lastDone % 2 === 0 || lastDone === users.length) renderWho();
      });
      renderWho();
    }catch(err){
      console.warn("user last two failed", err);
    }finally{
      refreshGuards.userLastTwo = false;
    }
  }
  ...
  async function refreshQueue(){
    ...
    try{
      ...
      state.queue = { ...state.queue, ...q };
      if(q.storeParsed && Date.now() - state.storeQueueLocalTouchTs >= 1400){
        state.storeQueue = { ...state.storeQueue, dealId: q.dealId, ...q.storeParsed, order: normalizeStoreOrder(q.storeParsed.order) };
      }
      renderQueueSidebar();
      renderStoreQueueSidebar();
    }catch(err){
  ...
  async function refreshStoreQueue(){
    if(refreshGuards.storeQueue) return;
    refreshGuards.storeQueue = true;
    try{
      if(Date.now() - state.storeQueueLocalTouchTs < 1400) return;
      if(!state.queue.dealId) return;
      const q = await fetchStoreQueue();
      state.storeQueue = { ...state.storeQueue, ...q, order: normalizeStoreOrder(q.order) };
      renderStoreQueueSidebar();
    }catch(err){
      console.warn("store queue failed", err);
      renderStoreQueueSidebar();
    }finally{
      refreshGuards.storeQueue = false;
    }
  }
  ...
  async function hardRefreshAll(){
    ...
    refreshMonthMetrics().catch(()=>{});
    refreshSuccess30Metrics().catch(()=>{});
    refreshUsersLastTwo().catch(()=>{});
    setStatus(`Atualizado: ${nowBRTime()}`);
  }
  ...
  function wire(){
    ...
    $("#storeQueueBody")?.addEventListener("click", async (e)=>{
      const up = e.target.closest("[data-sq-up]");
      const dn = e.target.closest("[data-sq-down]");
      const pause = e.target.closest("[data-sq-pause]");
      const play = e.target.closest("[data-sq-play]");
      if(!up && !dn && !pause && !play) return;
      try{
        const q = state.storeQueue.dealId ? state.storeQueue : await fetchStoreQueue();
        let order = normalizeStoreOrder(q.order);
        let paused = sanitizeStoreUserIdList(q.pausedStoreOnly);

        if(up || dn){
          const id = up ? up.getAttribute("data-sq-up") : dn.getAttribute("data-sq-down");
          const dir = up ? "up" : "down";
          state.storeQueue = { ...state.storeQueue, ...q, order, pausedStoreOnly: paused };
          order = moveStoreQueueLocal(id, dir);
          setStatus(`Fila loja ajustada • ${nowBRTime()}`);
        }else{
          const id = pause ? pause.getAttribute("data-sq-pause") : play.getAttribute("data-sq-play");
          const set = new Set(paused.map(String));
          if(pause) set.add(String(id));
          if(play) set.delete(String(id));
          paused = Array.from(set);
          setStatus(`${userNameById(id)} ${pause ? "pausada" : "reativada"} na loja • ${nowBRTime()}`);
        }

        await persistStoreQueue(order, paused);
      }catch(err){
        console.error(err);
      }
    });

    $("#btnStoreQueueWalk")?.addEventListener("click", async ()=>{
      try{
        const q = state.storeQueue.dealId ? state.storeQueue : await fetchStoreQueue();
        let order = normalizeStoreOrder(q.order);
        const paused = sanitizeStoreUserIdList(q.pausedStoreOnly);
        state.storeQueue = { ...state.storeQueue, ...q, order, pausedStoreOnly: paused };

        const active = activeStoreOrder();
        if(active.length === 0){
          alert("Nenhuma usuária ativa na fila presencial. Verifique a fila principal ou as pausas da loja.");
          return;
        }

        const nextId = active[0];
        order = order.filter(id=> String(id)!==String(nextId));
        order.push(String(nextId));

        setStatus(`Fila loja andou: ${userNameById(nextId)} • ${nowBRTime()}`);
        await persistStoreQueue(order, paused);
      }catch(err){
        console.error(err);
      }
  ...
  async function start(){
    ...
    await Promise.allSettled([refreshNewLeads(), refreshPendingCount(), refreshQueue(), refreshUsersFast()]);
    refreshMonthMetrics().catch(()=>{});
    refreshSuccess30Metrics().catch(()=>{});
    refreshUsersLastTwo().catch(()=>{});
    setStatus(`Atualizado: ${nowBRTime()}`);
    ...
    // Métricas gerais são atualizadas dentro de refreshUsersFast(), evitando chamadas duplicadas ao Bitrix.
    setInterval(refreshQueue, CONFIG.REFRESH_QUEUE_MS);
    // A fila presencial é carregada junto com a fila principal para não duplicar consultas ao Bitrix.
    setInterval(refreshStoreQueue, CONFIG.REFRESH_STORE_QUEUE_MS);
    setInterval(refreshUsersFast, CONFIG.REFRESH_WHO_MS);
    setInterval(refreshMonthMetrics, CONFIG.REFRESH_MONTH_METRICS_MS);
    setInterval(refreshSuccess30Metrics, CONFIG.REFRESH_SUCCESS30_MS);
    setInterval(refreshUsersLastTwo, CONFIG.REFRESH_USER_LAST_TWO_MS);
    setInterval(()=>{ stabilizeLogo().catch(()=>{}); }, 60000);
