...
(function(){
  ...
  const CONFIG = {
    ...
    SYSTEM_VERSION_LABEL: "Atualizado em 17/06/2026 20:18",
    SYSTEM_VERSION_CODE: "20260617-2018",
    LOGO_ASSET_VERSION: "20260617-2018",
  ...
  async function fetchCompanyLogoUrl(){
    ...
    try{
      ...
      if(cachedRaw){
        ...
        const maxAge = Math.max(5, Number(cfg.CACHE_MINUTES || 30)) * 60 * 1000;
        if(cached.url && age < maxAge) return cached.url;
      }
    }catch(_){}
    ...
    if(cfg.COMPANY_ID){
      try{
        company = await bx("crm.company.get", { id: String(cfg.COMPANY_ID) }, { timeoutMs: 8000 });
      }catch(err){
        console.warn("company logo get failed", err);
      }
    }
    ...
    if(!company){
      try{
        ...
        company = arr.find(c => !!extractLogoUrlFromValue(c.LOGO)) || arr[0] || null;
      }catch(err){
        console.warn("company logo list failed", err);
      }
    }

    const url = extractLogoUrlFromValue(company && company.LOGO);
    if(url){
      try{ localStorage.setItem(logoCacheKey(), JSON.stringify({ url, ts: Date.now() })); }catch(_){}
    }
    return url || "";
  }
    ...
    const useNext = ()=>{
      ...
      img.onload = ()=>{
        if(img.naturalWidth > 0 && img.naturalHeight > 0){
          ...
          img.dataset.logoSource = url === companyUrl ? "company" : "fallback";
          img.style.display = "block";
        }else{
