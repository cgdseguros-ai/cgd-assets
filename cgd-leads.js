    ...
    try{
      const run = ()=>{
        try{
          ...
          [0, 0.18, 0.36, 0.54].forEach((off, idx)=> leadBeep(ctx, base + off, 1040 + (idx*80), 0.14, 0.32));
        }catch(_ ){}
      };
      const ctx = getAudioCtx();
      if(ctx && ctx.state === "suspended") ctx.resume().then(run).catch(()=>{});
      else run();
    }catch(_ ){}
  }
...
.cgdGrid{ flex: 1 1 auto; display:grid; grid-template-columns: 1.15fr 1.85fr; gap: 12px; }
