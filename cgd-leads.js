...
async function modalBatchTransfer(){
  ...
  async function refreshUsersFast(){
    ...
    // Antes esta rotina fazia grandes varreduras com crm.lead.list para mês/30d e só depois renderizava.
    // Agora as métricas principais usam consultas de TOTAL (start:0/select ID) e são exibidas primeiro.
    // Os "últimos leads" das users entram depois, em paralelo limitado, para não travar os cards.
    ...
    try{
      ...
      // 1) Métricas gerais primeiro — leves e rápidas.
      ...
      // 2) Métricas por user — total direto por filtro, sem baixar milhares de leads do mês.
      state.dailyByUser = {};
      let done = 0;
      await pool(users, 4, async (u)=>{
        const uid = String(u.id);
        const [pulledToday, pulledMonth, att30, conv30] = await Promise.all([
          fetchPegCountRangeUser(u.id, dayS, dayE, CONFIG.COUNT_STATUS_ALLOWED),
          fetchPegCountRangeUser(u.id, monS, monE, CONFIG.COUNT_STATUS_ALLOWED),
        ...
        const pct = (Number(att30||0) > 0) ? Math.round((Number(conv30||0) / Number(att30||0)) * 100) : 0;
        state.dailyByUser[uid] = Number(pulledToday || 0);
        state.userStats[u.id] = {
          ...(state.userStats[u.id]||{}),
          pulledToday: Number(pulledToday || 0),
          pulledMonth: Number(pulledMonth || 0),
          success30: { attended: Number(att30||0), converted: Number(conv30||0), pct }
        };
        done++;
        if(done % 2 === 0 || done === users.length) renderWho();
      });
      renderWho();

      // 3) Informação complementar — últimos leads — carregada por último para não atrasar contagens.
      let lastDone = 0;
