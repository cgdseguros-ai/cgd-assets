...
(function(){
  ...
  function whipCrack(ctx, t){
    ...
    for(let i=0;i<data.length;i++){
      ...
      const decay = Math.exp(-x * 18);
      const bite = x < 0.05 ? (1 - (x / 0.05)) * 1.8 : 0;
      data[i] = ((Math.random() * 2 - 1) * decay * 0.95) + (Math.sin(i * 0.9) * bite * 0.45);
    }
    ...
    comp.threshold.setValueAtTime(-28, t);
    ...
    gain.gain.exponentialRampToValueAtTime(1.45, t + 0.003);
...
.cgdTitle{ font-weight: 950; letter-spacing:.2px; font-size: 12px; white-space: nowrap; }
.cgdTopRight{ display:flex; gap:6px; align-items:center; flex-wrap: wrap; justify-content: flex-end; }
