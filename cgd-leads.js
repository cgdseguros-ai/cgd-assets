...
(function(){
  ...
  // =========================
  // Audio — 3 beeps forte + unlock reforçado
  // =========================
  ...
  function leadBeep(ctx, t, freq=980, dur=0.13, gainLevel=0.26, type="square"){
    ...
    g.gain.exponentialRampToValueAtTime(gainLevel, t + 0.008);
    g.gain.exponentialRampToValueAtTime(0.18, t + Math.max(0.03, dur - 0.03));
    ...
    oscA.start(t);
    oscB.start(t);
    oscA.stop(t + dur + 0.03);
    oscB.stop(t + dur + 0.03);
  }
  ...
  function playLeadAlertFallback(){
    try{
      ...
      const fire = ()=>{
        ...
        leadBeep(ctx, base + 0.00, 1040, 0.12, 0.28, "square");
        leadBeep(ctx, base + 0.16, 1240, 0.12, 0.28, "square");
        leadBeep(ctx, base + 0.32, 1480, 0.18, 0.30, "square");
      };
  ...
  function playLeadAlert(){
    try{
      ...
      setTimeout(()=>{ if(!played) playLeadAlertFallback(); }, 180);
    }catch(_){
...
async function modalBatchTransfer(){
  ...
  async function refreshNewLeads(){
    ...
    try{
      if(!cgdAudioUnlocked) unlockAudio();
