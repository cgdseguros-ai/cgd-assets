...
(function(){
  ...
  const CONFIG = {
    ...
    // ✅ Para “atendidos do dia”: leads PEGOS no período que hoje estejam nestas etapas
    // (ATENDIDO + INTERAGIDO + QUALIFICADO)
...
body.cgdDark .cgdBadge{ background: rgba(255,255,255,.9) !important; }
  ...
  async function fetchPegAggregateRange(startISO, endISO, statuses){
    ...
    return { total: (rows || []).length, byUser, byUserStatus };
  }
...
async function modalBatchTransfer(){
  ...
  async function refreshUsersFast(){
    ...
    try{
      ...
      state.stats = { day: dayAgg.total || 0, month: monAgg.total || 0 };
      renderStats(state.stats);
