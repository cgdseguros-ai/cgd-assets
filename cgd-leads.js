...
#listWho.cgdWhoGrid{ display:grid !important; grid-template-columns: repeat(auto-fit, minmax(246px, 1fr)); gap: 10px; align-content:start; }
@media (max-width: 1100px){ #listWho.cgdWhoGrid{ grid-template-columns: repeat(auto-fit, minmax(232px, 1fr)); } }
.cgdDailyChart{
  margin: 10px 10px 0;
  border: 1px solid var(--border);
  border-radius: 16px;
...
body.cgdDark .cgdBadge{ background: rgba(255,255,255,.9) !important; }
  ...
  function renderDailyUserChart(){
    ...
    const rows = CONFIG.USERS.map(u=>({ ...u, n:Number(byUser[String(u.id)] || 0) }));
