...
(function(){
  ...
  async function unlockAudio(){
    try{
      ...
      cgdAudioUnlocked = (ctx.state === "running");
    }catch(_){}
  }
  ...
  function whipCrack(ctx, t){
    const dur = 0.24;
    const buffer = ctx.createBuffer(1, Math.max(1, Math.floor(ctx.sampleRate * dur)), ctx.sampleRate);
    const data = buffer.getChannelData(0);
    for(let i=0;i<data.length;i++){
      const x = i / data.length;
      const decay = Math.exp(-x * 10.5);
      const body = Math.sin(i * 0.045) * Math.exp(-x * 14) * 0.55;
      const snap = (Math.random() * 2 - 1) * decay * (x < 0.16 ? 1.45 : 0.78);
      const tail = (Math.random() * 2 - 1) * Math.exp(-Math.max(0, x - 0.12) * 7) * 0.34;
      data[i] = (snap + body + tail) * 0.92;
    }
    ...
    gain.gain.exponentialRampToValueAtTime(2.2, t + 0.006);
    gain.gain.exponentialRampToValueAtTime(0.42, t + 0.05);
  ...
  function runHorseAcross(){
    ...
    try{
      ...
      d.innerHTML = `
        <svg viewBox="0 0 420 220" width="420" height="220" aria-hidden="true">
          <g class="horseBob">
            <ellipse cx="205" cy="112" rx="88" ry="44" fill="#b67a3d" stroke="rgba(60,30,10,.48)" stroke-width="5"/>
            <ellipse cx="300" cy="92" rx="38" ry="28" fill="#b67a3d" stroke="rgba(60,30,10,.48)" stroke-width="5"/>
            <path d="M325 86 L358 70 L343 100 Z" fill="#8a5626"/>
            <path d="M286 108 Q255 58 220 54" fill="none" stroke="#7b4a22" stroke-width="12" stroke-linecap="round"/>
            <circle cx="313" cy="88" r="4.5" fill="#1f160f"/>
            <path d="M325 104 Q312 113 295 111" fill="none" stroke="#4e2f17" stroke-width="4" stroke-linecap="round"/>
            <path d="M120 116 Q96 102 82 84" fill="none" stroke="#7b4a22" stroke-width="10" stroke-linecap="round"/>
            <path d="M174 150 L158 204" stroke="#6c411d" stroke-width="10" stroke-linecap="round" class="leg leg1"/>
            <path d="M230 150 L245 204" stroke="#6c411d" stroke-width="10" stroke-linecap="round" class="leg leg2"/>
            <path d="M267 145 L288 202" stroke="#6c411d" stroke-width="10" stroke-linecap="round" class="leg leg3"/>
            <path d="M142 145 L120 202" stroke="#6c411d" stroke-width="10" stroke-linecap="round" class="leg leg4"/>
            <ellipse cx="205" cy="118" rx="53" ry="18" fill="rgba(255,255,255,.14)"/>
            <path d="M160 126 Q205 142 252 126" fill="none" stroke="rgba(60,30,10,.22)" stroke-width="6" stroke-linecap="round"/>
          </g>
          <g class="dust">
            <ellipse cx="115" cy="206" rx="18" ry="7" fill="rgba(160,120,80,.23)"/>
            <ellipse cx="145" cy="210" rx="24" ry="8" fill="rgba(160,120,80,.18)"/>
            <ellipse cx="260" cy="208" rx="20" ry="7" fill="rgba(160,120,80,.22)"/>
          </g>
        </svg>
      `;
      document.body.appendChild(d);
      setTimeout(()=>{ try{ d.remove(); }catch(_){} horseRunBusy = false; }, 2200);
...
.cgdTitle{ font-weight: 950; letter-spacing:.2px; font-size: 11px; white-space: nowrap; }
.cgdTopRight{ display:flex; gap:5px; align-items:center; flex-wrap: wrap; justify-content: flex-end; }
...
.cgdQueueArrows{ display:flex; gap:6px; align-items:center; justify-content:flex-end; align-self:center; }
.cgdQueueArrows .cgdMiniBtn{ min-width: 34px; padding: 6px 8px; line-height: 1; display:inline-flex; align-items:center; justify-content:center; }
...
.cgdHorseRun{
  ...
  pointer-events:none;
  opacity: .99;
  animation: horseRunAcross 1.95s linear forwards;
}
.cgdHorseRun .horseBob{ transform-origin: 50% 55%; animation: horseBob .22s ease-in-out infinite alternate; }
.cgdHorseRun .leg{ transform-origin: center top; animation: horseLeg .18s ease-in-out infinite alternate; }
.cgdHorseRun .leg2, .cgdHorseRun .leg4{ animation-direction: alternate-reverse; }
.cgdHorseRun .dust{ animation: horseDust .28s ease-in-out infinite alternate; }
...
@keyframes horseDust{
  ...
  100%{ transform: translateY(-4px) scale(1.08); opacity: .18; }
}
...
async function modalBatchTransfer(){
  ...
  async function refreshNewLeads(){
    ...
    try{
      ...
      if((all||[]).length > 0){
        if(hasNewArrival && !state.newLeadBlinkKey.has(Array.from(nextIds).sort().join(","))){
          state.newLeadBlinkKey.add(Array.from(nextIds).sort().join(","));
          runHorseAcross();
          if(state.soundOn){
            playWhipAlert();
            setTimeout(()=>{ if(state.soundOn && (state.newLeadsAll||[]).length > 0) playWhipAlert(); }, 120);
          }
