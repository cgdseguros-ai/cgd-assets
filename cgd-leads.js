...
body.cgdDark .cgdBadge{ background: rgba(255,255,255,.9) !important; }
  ...
  // State (RAM)
  // =========================
  const refreshGuards = { newLeads:false, pending:false, stats:false, users:false, queue:false, storeQueue:false };
      ...
      <div id="cgdApp">
        <div class="cgdTop">
          <div class="cgdTopLeft">
            <img class="cgdLogo" src="${esc(CONFIG.LOGO_URL)}" alt="CGD" referrerpolicy="no-referrer" loading="eager" onerror="this.onerror=null;this.src='${esc(CONFIG.LOGO_FALLBACK_URL)}';" />
...
async function modalBatchTransfer(){
  ...
  async function refreshUsersFast(){
    ...
    // ✅ V4: fluxo estável e mais leve.
    // 1) Renderiza os cards imediatamente e nunca deixa a área em branco.
    // 2) Carrega os números gerais primeiro.
    // 3) Usa 3 agregações por período para preencher cards/gráfico, em vez de dezenas de consultas por USER.
    // 4) Últimos leads entram por último, sem derrubar os cards.
    ...
    try{
      ...
      // 1) Métricas superiores primeiro — consultas de total, rápidas.
      try{
        const [dayTotal, monthTotal, lostMonth, junkMonth, convMonth] = await Promise.all([
          fetchPegCountRangeAll(dayS, dayE, CONFIG.COUNT_STATUS_ALLOWED),
        ...
        state.monthlyByStatus = {
          [CONFIG.LEAD_STATUS.PERDIDO]: lostMonth || 0,
          [CONFIG.LEAD_STATUS.LEAD_DESCARTADO_SISTEMA]: junkMonth || 0,
          [CONFIG.LEAD_STATUS.LEAD_CONVERTIDO_SISTEMA]: convMonth || 0
        };
        state.stats = {
          day: dayTotal || 0,
          month: monthTotal || 0,
          pctLost: pctPart(lostMonth, monthTotal),
          pctJunk: pctPart(junkMonth, monthTotal),
          pctConverted: pctPart(convMonth, monthTotal)
        };
        renderStats(state.stats);
      }catch(err){
        console.warn("top stats failed", err);
      }

      // 2) Cards/gráfico por USER — agregação única por período, sem 4 consultas por user.
      try{
        ...
        console.warn("user aggregate stats failed", err);
        renderWho();
      }

      // 3) Informação complementar — últimos leads — carregada por último.
      let lastDone = 0;
      await pool(users, 3, async (u)=>{
        try{
        ...
        lastDone++;
        if(lastDone % 3 === 0 || lastDone === users.length) renderWho();
      });
      renderWho();
    }catch(err){
