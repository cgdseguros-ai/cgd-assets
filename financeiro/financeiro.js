...
(function () {
  ...
  function loadAudit() {
    try {
      var a = parseJson(localStorage.getItem(CFG.LS.AUDIT) || "[]");
      S.audit = Array.isArray(a) ? a : [];
    } catch (_) { S.audit = []; }
  }
  function saveAudit() {
    try { localStorage.setItem(CFG.LS.AUDIT, JSON.stringify((S.audit || []).slice(0, 300))); } catch (_) {}
  }

  function loadFilterPresets() {
    try {
      var arr = parseJson(localStorage.getItem(CFG.LS.FILTER_PRESETS) || '[]');
      S.filterPresets = Array.isArray(arr) ? arr : [];
    } catch (_) { S.filterPresets = []; }
  }
  function saveFilterPresets() {
    try { localStorage.setItem(CFG.LS.FILTER_PRESETS, JSON.stringify((S.filterPresets || []).slice(0, 50))); } catch (_) {}
  }
  function loadLastFilters() {
    try {
      var raw = parseJson(localStorage.getItem(CFG.LS.LAST_FILTERS) || '{}') || {};
  ...
  function saveLastFilters() {
    try { localStorage.setItem(CFG.LS.LAST_FILTERS, JSON.stringify(S.filters || {})); } catch (_) {}
  }
  ...
  function render() {
    ...
    el("#btn-bulk").addEventListener("click", openBulkActionsModal);
    el("#btn-audit").addEventListener("click", openAuditModal);
    el("#btn-save-filter").addEventListener("click", saveCurrentFilterPreset);
    el("#btn-filters").addEventListener("click", openFilterPresetsModal);
    el("#btn-reset-filters").addEventListener("click", resetAllFilters);

    el("#btn-refresh").addEventListener("click", refresh);

    el("#f-q").addEventListener("input", function (e) { debounceApplySearch(e.target.value || ""); });
    ...
    el("#fin-select-all").addEventListener("change", function () {
      var list = getPagedList();
      var on = !!el("#fin-select-all").checked;
