...
(function () {
  ...
  function syncSelectAllState() {
    ...
    if (!box) return;
    var list = S.filtered || [];
    var total = Math.min(list.length, CFG.PAGE_SIZE);
    var count = 0;
    ...
    m.q('#bulk-go').addEventListener('click', function(){
      ...
      ids.forEach(function(id){
        chain = chain.then(function(){
          ...
          if (action === 'delete') {
            return deleteDeal(id).then(function(){ removeDealLocal(id); updated++; });
          }
          if (action === 'conta') {
            var conta = String(m.q('#bulk-conta').value || '');
            var fieldsConta = {}; fieldsConta[CFG.F.CONTA] = conta;
            return updateDeal(id, fieldsConta).then(function(){ mergeDealPatch(id, fieldsConta); updated++; });
          }
          if (action === 'centro') {
            var centro = String(m.q('#bulk-centro').value || '');
            var fieldsCentro = {}; fieldsCentro[CFG.F.CENTRO_CUSTO] = centro;
            return updateDeal(id, fieldsCentro).then(function(){ mergeDealPatch(id, fieldsCentro); updated++; });
          }
          if (action === 'stage') {
            ...
            return updateDeal(id, fieldsStage).then(function(){ mergeDealPatch(id, fieldsStage); updated++; });
          }
        });
      });
